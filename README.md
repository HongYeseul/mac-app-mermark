...
- **태그** — 본문의 `#태그`와 프론트매터 `tags:`를 모아 사이드바에 보여주고, 누르면 그 태그만 걸러 봅니다.
  프리뷰에서는 알약 모양으로 눈에 띄게 표시되고, 눌러도 같은 필터가 걸립니다
...
지금 쓰는 폴더 경로는 창 제목 아래에 늘 보입니다. 사이드바에서 노트를 우클릭하면
Finder에서 바로 열 수 있고, `⌘⇧R`은 노트 폴더를 엽니다.
...
| 사이드바 태그 | 누르면 그 태그만, 다시 누르면 전체 (검색과 함께 적용됨) |
| 사이드바 노트 우클릭 | Finder에서 보기 · 경로 복사 |
| `⌘⇧R` | 노트 폴더를 Finder에서 보기 |
