...
- **노트 지우기** — 사이드바나 탭에서 우클릭해 휴지통으로 보냅니다. 바로 지우지 않으므로 되돌릴 수 있습니다
...
| 사이드바 작업 공간 우클릭 | Finder에서 보기 · 경로 복사 · 연결 해제 |
...
| 스위트 | 검증 내용 |
|---|---|
| `note-store` | 실파일·실 FSEvents 기반 작업 공간 동작과 검색·다중 연결 케이스 |
...
| `trash` | 실제로 휴지통에 옮겨지는지와 탭·선택 정리 16 케이스 |
